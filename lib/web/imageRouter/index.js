...
imageRouter.post('/uploadimage', function (req, res) {
  ...
  form.parse(req, async function (err, fields, files) {
    if (err) {
      ...
      const imageUrl = new URL(fields.image_url)
      const ext = imageUrl.pathname.split('.').pop()
      const filename = uuidv4() + '.' + (ext ? ext : "invalid")
      const dest = path.join(tmpDir, filename)

      request(fields.image_url).pipe(fs.createWriteStream(dest)).on('close', () => {
        logger.debug(`SERVER downloaded image: ${dest}`)

        const uploadProvider = require('./' + config.imageUploadType)
        logger.debug(`imageRouter: Uploading ${dest} using ${config.imageUploadType}`)
        uploadProvider.uploadImage(dest, function (err, url) {
          rimraf.sync(tmpDir)
          if (err !== null) {
            logger.error(err)
        ...
        logger.error(err)
        rimraf.sync(tmpDir)
        return res.status(500).end('download image error')
      })
